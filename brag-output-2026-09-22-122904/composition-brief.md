# Hyperframes Composition Brief: hline (narrated cut, cursor-accurate multi-select)
...
## Output
- Composition directory: `brag-output-2026-09-22-122904/composition/`
- Rendered video: `brag-output-2026-09-22-122904/brag.mp4`
...
- Key UI to recreate: the ratatui TUI from `src/ui.rs` — a bordered list box titled `hline`, rows prefixed `[ ]` with `YYYY-MM-DD HH:MM` timestamps, the cursor row drawn with `> ` and reversed video, an amber search line, and a dark-gray status bar. The cursor row is the row `Space` acts on: it must move with the selection, never sit on a row that is never checked.
...
## Storyboard
Use the storyboard in `brag-output-2026-09-22-122904/brag-plan.md` as the creative contract.
...
4. Favorite it — 10.2-14.6s — the cursor walks the filtered list and `Space` checks the row it is on (10.93 / 11.46 / 12.02, moves at 11.30 / 11.86), `saved favorite` at 13.11s, flip to favorites, `vo4`.
